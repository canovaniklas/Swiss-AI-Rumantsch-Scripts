...
def convert_romansh_sft(row: Dict[str, Any], dataset_source: str) -> Dict[str, Any]:
    """
    Convert Romansh SFT format (t_question/t_answer).
    Used for: Your custom combined.csv data.
    """
    question = row.get("t_question")
    answer = row.get("t_answer")

    if not question:
        raise ValueError("Missing required field: t_question")

    conversation_id = generate_conversation_id(dataset_source, question)
    ...
    original_metadata = {k: v for k, v in row.items() 
                         if k not in ["t_question", "t_answer"]}
...
CONVERTERS = {
    ...
    "romansh_sft": convert_romansh_sft,
}
...
Examples:
  ...
  python convert_to_chat_format.py ../data/01-hf-data/romansh_sft ../data/02-standardised/ --force-format romansh_sft
        """
    )
